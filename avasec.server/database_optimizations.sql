...
-- ============================================
-- 6. DATABASE MAINTENANCE QUERIES
-- ============================================

-- Clean up old notifications (older than 30 days and read)
-- Run this periodically as maintenance
-- DELETE FROM Notifications WHERE IsRead = TRUE AND CreatedAt < DATE_SUB(NOW(), INTERVAL 30 DAY);

-- Clean up old user activity logs (older than 90 days)
-- DELETE FROM UserActivity WHERE ActivityDate < DATE_SUB(NOW(), INTERVAL 90 DAY);

-- Clean up old system logs (older than 30 days)
-- DELETE FROM SystemLogs WHERE CreatedAt < DATE_SUB(NOW(), INTERVAL 30 DAY);
